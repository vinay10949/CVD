...
from cvd_model.processing.data_management import load_dataset
...
def populate_database(n_predictions: int = 500, anomaly: bool = False) -> None:
    ...
    for index, data in clean_inputs_df.iterrows():
        ...
        response = requests.post(
            f"{LOCAL_URL}/v1/predictions/predict",
            headers=HEADERS,
            json=[data.to_dict()],
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Send random requests to Cardio Vascular Detection API.')
